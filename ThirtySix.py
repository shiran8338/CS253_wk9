...
leet_mask = {'a': '@', 'b': '8', 'c': '{', 'd': 'd', 'e': '3', 'f': 'f', 'g': '6', 'h': '#', 'i': '!', 'j': '/', 'k': 'k', 'l': '|',
        'm': 'm', 'n': 'n', 'o': '0', 'p': 'p', 'q': '9', 'r': 'r', 's': '$', 't': '7', 'u': 'u', 'v': 'v', 'w': 'w', 'x': '%', 'y': 'y', 'z': '2'}
...
def input_generator(nsamples):
    def generate_line():
        ...
        for _ in range(nsamples):
            ...
            updated_expected = leet_mask[expected] if expected in leet_mask else expected
            inline.append(c); outline.append(updated_expected)
        return ''.join(inline), ''.join(outline)

    while True:
        input_data, updated_expected = generate_line()
        data_in = encode_one_hot(input_data)
        data_out = encode_one_hot(updated_expected)
